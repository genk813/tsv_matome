...
## 2025-08-16 TMSONARパリティ向上 - コードマッピング完全実装

### 実施内容
1. **DOCXファイルベースのコード定義抽出と実装**
   - 4-02出願マスタ（意商）_1.13版.docxから以下のコード定義を抽出：
     - 審査種別（02310）: 4コード（通常審査、早期審査、審判、再審）
     - 査定種別（02190）: 2コード（登録査定、拒絶査定）
     - 適用法規区分（02440）: 11コード（商標法改正履歴）
   
   - 4-18共有データベース（審判）_1.13版.docxから：
     - 審判種別（18190-18210）: 32コード（△記号付き、半角スペース表記）
     - 審判最終処分（18220）: 13コード
     - 異議最終処分（18320）: 10コード
   
   - 4-12商標基本マスタ_1.13版.docxから：
     - 訴訟コード（02360）: 6コード
     - 公序良俗違反フラグ（02150）: 2コード
     - 各種フラグ（標準文字、特殊商標、色彩等）: 0/1定義

2. **コード変換関数の統合**
   - `_convert_code_to_name`関数を拡張し、全15種類のコードマッピングに対応
   - appeal_type処理の追加（当初漏れていた）
   - フラグ系5種類の新規対応

3. **実装テスト結果**
   - tmcloud_v2_20250807_213449.dbで動作確認
   - 審査種別'03'が正しく「審判」に変換（474件該当）
   - 審判種別'80'が「査定不服審判」に変換
   - 適用法規区分が商標法改正年に変換

4. **中間記録コードの確認**
   - 既存実装の統計：合計6,566件
   - 4カテゴリ（審査882件、審判850件、登録3,956件、マドプロ878件）で体系化済み

5. **その他の改善**
   - 日付フォーマット関数（_format_date）追加
   - ウィーンコード検索は既に実装済み（階層的前方一致対応）

### 重要な発見
- 審査種別'03'は「早期審査」ではなく「審判」が正しい（DOCXで確認）
- 審判種別コードは12000番台ではなく18000番台（18190-18210）
- △記号は半角スペースを表す（DOCX仕様）

### 作業プロセスの改善
- DOCXファイル → コードINDEX → 実装の正しい手順を確立
- データベースから推測するのではなく、公式仕様書を参照する方針を徹底

---

## 2025-08-16 (続き) 追加機能実装と問題修正

### 実施内容

1. **分納案件の次回納付期限表示修正**
   - 問題：`next_pen_payment_limit_date`が分納でない案件にも表示されていた
   - 原因：`installments_id`フィールドの判定が欠落
   - 修正：`installments_id = '1'`の場合のみ表示するよう条件追加
   - 該当コード：tmcloud_search_integrated.py line 10924-10927

2. **ウィーンコード表示の復活**
   - 問題：バックエンドでは取得していたがフロントエンドで表示されていなかった
   - 修正：tmcloud_simple_web.py line 413に表示ロジック追加
   - 実装：`addArrayField('ウィーンコード', info.vienna_codes)`

3. **審決分類と拒絶条文コードの実装**
   - データソース特定：
     - 審決分類：`trademark_appeal_cases.appeal_final_disposition_code`
     - 拒絶条文：`trademark_draft_records.rejection_reason_code`
   - SQLクエリ拡張：rejection_dataとappeal_dataのCTE追加
   - コードマッピング実装：
     - 拒絶条文コード（C0710）：92種類のマッピング定義
     - 審決分類コード（18350）：20種類のマッピング定義
   - 変換関数追加：
     - `_format_rejection_codes()`: 拒絶条文コードを日本語条文に変換
     - `_format_appeal_disposition_codes()`: 審決分類を日本語に変換
   - Web表示追加：line 415-419

4. **商標タイプと詳細説明の実装**
   - 商標タイプ判定ロジック：
     - 団体商標：`group_t_flg = '1'`
     - 地域団体商標：`area_group_t_flg = '1'`
     - 標準文字商標：`standard_char_exist = '1'`
     - 立体商標、音商標等：`special_mark_exist`で判定
   - 詳細説明：`trademark_detailed_descriptions`テーブルから取得

5. **拒絶条文コードの修正**
   - コード「55」：「第８条第２項」（同日出願）に修正
   - コード「95」：「その他＋その他」→「その他」に修正
   - 重複定義の削除（REJECTION_REASON_CODE_MAP_DELETEDに変更）

### 類似群コードの表示問題の調査結果

1. **システムは正常動作している**
   - データが存在する案件では正しく表示される（例：2023046258）
   - 検索・取得・表示のロジックに問題なし

2. **根本原因はデータ不足**
   - 全案件（61,769件）中、類似群コードあり：33,398件（54.1%）
   - 2024年案件：30,861件中11,815件（38.3%）
   - 2025年案件：28,659件中4,470件（15.6%）
   - 指摘案件（2025060121、2024125769）：データベースに類似群コードが存在しない

3. **データ不足の原因**
   - 特許庁の週次更新TSVファイルに類似群コードが完全に含まれていない
   - 初期データ（2025年6月）と週次差分データの不完全性

### テスト結果
- 拒絶条文コード「92」→「第３条柱書＋その他（第４条１項１１号を除く）」
- 審決分類コード「01」→「請求成立」
- 審決分類コード「15」→「審理終結」

### 技術的変更点
- パラメータ数の調整：batch_app_nums * 20（rejection_data追加前は21になっていたが20が正しい）
- APPEAL_DISPOSITION_MAP_DELETEDに重複定義を移動
- 不要なMDファイル10個を削除し、ドキュメント整理
...
#### 1. 審判中間コードの完全マッピング
- **問題**: 審判中間記録が正しく表示されない、未マッピングコードが多数存在
- **調査**: 04.docx のINDEX C0850（審判中間コード）から425個のコードを抽出
- **修正内容**:
  - C116: 手続補正指令書（請求）（審判長）
  - C126: 手続補正指令書（中間）（審判長）
  - C2829: 証拠調べ通知書
...
#### 2. 登録中間コードの「〃」記号問題の修正
- **問題**: 登録中間コード（C0860）に「〃」（同上）記号が2,188箇所含まれていた
- **原因**: DOCXファイルからのコピー時に「〃」がそのまま貼り付けられていた
- **修正**: すべての「〃」を実際の値に置換（例：R121〃 → R121210）

#### 3. 重複コード定義の削除
- **問題**: 同じコードマッピングが複数箇所で定義されていた
- **削除した重複定義**:
  - REGISTRATION_INTERMEDIATE_CODES（1,978行）
  - TRIAL_INTERMEDIATE_CODE_MAP（893行）
- **結果**: 2,871行削除、ファイルサイズ削減

#### 4. 審査中間記録の並び順修正
- **問題**: 審査中間記録が新しい順（DESC）で表示されていた
- **修正**: ORDER BY record_date ASC に変更
- **結果**: 古い順（願書→拒絶理由通知書→拒絶査定）で正しく表示

#### 5. SQL構文エラーの修正
- **問題**: GROUP BY前にORDER BY句があり構文エラー
- **修正**: trial_intermediate_docs CTEでORDER BY句を削除
- **結果**: エラー解消、Web検索が正常動作

#### 6. コードマッピングの全角・半角統一
- **問題**: 全角・半角の重複で964個の無駄なエントリ
- **修正内容**:
  - EXAMINATION_TYPE_MAP: 4個の重複削除
  - LITIGATION_CODE_MAP: 6個の重複削除
  - OPPOSITION_DISPOSITION_MAP: 10個の重複削除
  - APPLICABLE_LAW_CLASS_MAP: 11個の重複削除
  - EXAMINATION_DISPOSITION_MAP: 34個の重複削除
  - INTERMEDIATE_CODE_MAP: 440個の重複削除
  - MADRID_INTERMEDIATE_CODE_MAP: 439個の重複削除
- **結果**: 
  - 合計964個の重複エントリを削除
  - ファイルサイズ削減
  - 保守性向上
...
#### 動作確認結果
- 2024022742（4件の審査中間記録）: ✓ 正しく表示
- 2022145365（31件の審査中間記録）: ✓ 30/31件が日本語表示
- 審判中間記録: ✓ 正しくマッピング
- 登録中間記録: ✓ 構造は用意（データなし）
- 並び順: ✓ 古い順で統一
- Web UI: ✓ エラーなく動作
...
#### 問題の発見
- **現象**: 審判中間記録と登録中間記録が表示されない
- **原因**: SQLクエリが`trademark_application_records`と`trademark_draft_records`からしか取得していなかった
- **調査**: データベースの実際のテーブル構造を確認
  - 審判データ: `trial_cases`, `trial_received_docs`, `trial_dispatched_docs`
  - 登録データ: `trademark_progress_info`

#### 修正内容
1. **progress_data CTEの拡張**
   - 審判中間記録の追加:
     - `trial_cases`から審判請求日を`C60`コードとして取得
     - `trial_received_docs`から受領文書を取得
     - `trial_dispatched_docs`から発送文書を取得
   - 登録中間記録の追加:
     - `trademark_progress_info`から`reg_intermediate_code`を取得
   
2. **パラメータ数の調整**
   - 修正前: 14個のplaceholders
   - 修正後: 18個のplaceholders（審判3個、登録1個追加）

#### 審判中間コードのC付き問題
- **経緯の複雑さ**:
  1. 最初: DOCXから抽出、ハイフン付き（11-6）→C付き（C116）、数字のみ（60）→そのまま
  2. 誤修正: 数字のみにもC付けた（03→C03）→ユーザー指摘で削除
  3. 実態: データベースには全てC付き（C60など）で格納
  4. 最終解決: 数字のみコードとC付きバージョンの両方をマッピング

#### コードマッピングの完全化
- **作業内容**: TRIAL_INTERMEDIATE_CODE_MAPの全数字コードにC付きバージョンを追加
  - 03と`C03`: "審決"
  - 60と`C60`: "審判請求書（その他の請求書・申立書を含む）"
  - その他51個の数字コードすべてに対応
- **結果**: データベースのC付きコードも数字のみコードも両方正しくマッピング

#### 最終動作確認
- **審査中間記録**: ✓ 4件正しく表示（2024022742）
- **審判中間記録**: ✓ 13件正しく表示、C60も「審判請求書」として日本語表示（2009084662）
- **登録中間記録**: ✓ 7件正しく表示（2024012592）
- **サーバー**: http://localhost:5000 で正常稼働

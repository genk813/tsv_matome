...
## 2025-08-14 TMSONARの一覧表示モード項目実装と中間記録分類修正

### TMSONARパリティ向上作業
1. **TMSONAR一覧表示モードの調査**
   - 検索結果HTMLファイルから実際の表示項目を確認
   - 以下の未実装項目を発見：
     - 最終処分記事（SB_KIJI）
     - 審判条文記事（SPJ_KIJI）
     - 商標画像サムネイル（LSTIMG）
...
2. **最終処分記事の実装**
   - 最終処分コードを日本語記事に変換する機能を追加
   - `FINAL_DISPOSITION_CODES`辞書（A01:登録査定、A02:拒絶査定等）を使用
   - `_format_final_disposition()`メソッドを実装
   - CLIツールとWeb UIの両方で表示可能に

3. **審判条文記事について**
   - 特許庁CSVファイル仕様を調査した結果、該当データが存在しないことが判明
   - TMSONARのHTMLでも実際にはデータが空白
   - データソース不在のため実装不可と判断

4. **中間記録の分類表示修正**
   - 問題：中間記録が審査・審判・登録に正しく分類されていない
   - 原因：日本語変換後のテキストで分類していたため判定不能
   - 解決：バックエンドで分類してから返すように修正
   - `_format_progress_records()`が辞書形式で分類済みデータを返すように変更
   - Web UIも新しい形式に対応

### 技術的詳細
- 審判条文コードマッピング`APPEAL_ARTICLE_CODE_MAP`を追加（将来用）
- 中間記録の分類ロジック：
  - A系およびIB/MD/AP/M3系 → 審査中間記録
  - 数字で始まるコード → 審判中間記録
  - R系 → 登録中間記録
